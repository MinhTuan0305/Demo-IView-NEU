...
export async function GET(_: Request, ctx: { params: Promise<{ filename: string }> | { filename: string } }) {
  ...
  let res = await fetchText(`${BASE}/api/view_result?hint=${encodeURIComponent(name)}`);
  if (!res.ok) {
    // 2) resolve
    const rr = await fetch(`${BASE}/api/resolve_result_file?hint=${encodeURIComponent(name)}`);
    if (rr.ok) {
      const j = await rr.json();
      const match = j?.match as string | undefined;
      if (match) {
        res = await fetchText(`${BASE}/api/view_result?hint=${encodeURIComponent(match)}`);
        if (res.ok) {
    ...
    const list = await fetchText(`${BASE}/api/results`);
    if (list.ok) {
      try {
        ...
        if (best) {
          const r2 = await fetchText(`${BASE}/api/view_result?hint=${encodeURIComponent(best)}`);
          if (r2.ok) {
